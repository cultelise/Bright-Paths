...
//My formatter automatically puts it on the second line, but there are no curly braces.

const oneLiner = (p1, p2) =>
  console.log(`The first parameter is ${p1}. The second parameter is ${p2}.`);
...
const greeting = (firstName, lastName, cbFunc) => {
  cbFunc(firstName + ' ' + lastName);
};

// INVOKE 'GREETING FUNCTION' HERE

greeting('Elise', 'Heard', (fullName) =>
...
const pricesPlusTax = (pricesArr, cbFunc) => {
  for (let i = 0; i < pricesArr.length; i++) {
    const price = pricesArr[i];
    const taxedPrice = price * 1.2;
    totalCost.push(taxedPrice);
  }
  cbFunc(totalCost);
};
