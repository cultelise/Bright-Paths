...
test('can search Google twice', async () => {
	...
	await driver.findElement(By.name('q')).sendKeys('Belarus', Key.RETURN);
	await driver.wait(until.titleIs('Belarus - Google Search'), 1000);
	...
	await driver.findElement(By.name('q')).sendKeys('Tiger', Key.RETURN);

	// TODO Wait for the results page to load
	await driver.wait(until.titleIs('Tiger - Google Search'), 1000);
});
